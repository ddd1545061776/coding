...
!function($) {
    ...
    ChartJs.prototype.respChart = function(selector,type,data, options) {
        ...
        ChartJs.prototype.init = function(chartData) {
                ...
                datasets: [
                    {
                        ...
                        pointRadius: 1,
                        pointHitRadius: 10,
                        data: chartData.lineData.weekNum
                    }
                ]
            };
            ...
            //barchart
            var barChart = {
                labels: chartDate.barData.class,
                // data:chartDate.lineData,
                datasets: [
                    {
                        label: "通过人数",
                        ...
                        data: chartDate.barData.successNum
                    },
                    {
                        label: "提交人数",
                        ...
                        hoverBorderColor: "#0e86e7",
                        data: chartDate.barData.submitNum
                    }
                ]
            };
            var barOpts = {
...
//initializing
    $(document).ready(function () {
        ...
        // 获取折线题目提交次数数据
        $.ajax({
            url: baseUrl + '/question-chart-line'+order,
            type: 'post',
            contentType: 'application/json',
            async: false,
            success: function (res) {
                let lineData = JSON.parse(res)
                chartDate.lineData = lineData;
                $("#allNum").text(lineData.allNum);
                $("#sucNum").text(lineData.successNum);
            }
        })
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                chartDate.barData = resData;
                var html=""
                for (let i = 0; i <resData.allClass.length; i++) {
                    html+="<li class=list-inline-item><h5 id=computer-num>"+resData.allClass[i].studentNums+"</h5>" +
                        "<p class=text-muted\">"+resData.allClass[i].className+"</p></li>"
                }
                $("#urli").append(html)
