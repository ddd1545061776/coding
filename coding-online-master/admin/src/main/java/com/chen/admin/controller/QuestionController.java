...
//        return JSON.toJSONString(judgeResult);
    ...
    public String getInputType() {
        List<String> allType = questionTypeService.getAllInputType();
        return JSON.toJSONString(allType);
    }
...
//    }
    ...
    public String questionSubmit(@RequestBody Map<String, Object> map) {
        ...
        if (!StringUtils.hasLength(inputType)) {
            throw new BadArgumentException("参数一类型不能为空！");
        }

        String inputType1 = (String) map.get("inputType1");
//        if (!StringUtils.hasLength(inputType1)) {
//            throw new BadArgumentException("参数二类型不能为空！");
//        }
        ...
        String inputTypeId = inputTypeService.selectIdByTypeName(inputType);
        String inputTypeId1="";
        if (!inputType1.equals("")){
            inputTypeId1 = inputTypeService.selectIdByTypeName(inputType1);
        }
        String ss="";
        if (inputTypeId1.equals("")){
            ss=inputTypeId;
        }else {
            ss =inputTypeId+"#"+inputTypeId1;
        }
